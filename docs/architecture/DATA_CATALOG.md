...
| `domains/knowledge/` | `personal_assistant.storage` | Schema 3: Dokumente, Chunks, generisches und feldgetrenntes Mail-FTS, Sync-Cursor sowie additive Mail-Generationen, Contents, Occurrences, Locator, aktive/inaktive Tagprovenienz und Threadkanten | SQLite-Backup-API; idempotente additive Migration; Sync `rw`, Gateway/Monitor `ro`; getrennt von ActionPlan/Audit |
...
| `domains/knowledge/knowledge.sqlite3` | Gateway/CLI, Sync | Suche, Monitor `ro` | Dokumente, Chunks, feldgetrenntes BM25-Mail-FTS, geschlossene Tags und Sync-Cursor bilden einen konsistenten SQLite-Satz; Struktur-Tags stammen aus Parser/Locator, vorhandene Kategorie-, Review- und Extraktorfakten read-only aus `mail_agent.sqlite3`; eine komplette v2-Generation und ihr Cursor werden gemeinsam committed; Querymetriken enthalten keine Suchtexte, Adressen oder Snippets |
