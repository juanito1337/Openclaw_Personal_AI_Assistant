# Historischer README-Stand vor M1: OpenClaw 3.4.0-r27.2.5

> Archivierte Release- und Funktionsbeschreibung. Dieses Dokument ist kein
> aktueller Architektur- oder Betriebsvertrag. Der verbindliche Einstieg liegt
> im Repository-README, chronologische Aenderungen stehen im `CHANGELOG.md` des
> Repository-Roots.
...
Der verbindliche lokale und CI-Prueflauf, die Testcollection, Coverage-Baseline,
Quellmanifest-Regeln und gepinnten Werkzeugversionen stehen in
[`docs/TESTING.md`](docs/TESTING.md). Wheel-, Image- und CI-Nachweise beschreibt
[`docs/BUILD_AND_CI.md`](docs/BUILD_AND_CI.md). Der kurze Einstieg ist:
...
Diese Befehle verwenden nur den Checkout und die neutrale Beispielkonfiguration;
sie starten keine produktiven Jobs und greifen nicht auf `/srv/openclaw` zu.
...
The application remains one modular product and is deployed as a multi-process
container stack. Gateway and specialized workers have separate lifecycles, but are
not independent agents. SQLite stores metadata, index state, ActionPlans, and audit
entries. Original documents remain in Nextcloud.

The current runtime and rollback design is documented in
[`docs/DOCKER_DEPLOYMENT.md`](docs/DOCKER_DEPLOYMENT.md). The audited, incremental
modernization plan is in
[`docs/CONTAINER_ARCHITECTURE_ROADMAP.md`](docs/CONTAINER_ARCHITECTURE_ROADMAP.md).
